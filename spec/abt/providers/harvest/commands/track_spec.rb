...
RSpec.describe(Abt::Providers::Harvest::Commands::Track, :harvest) do
  ...
  let(:user_id) { '1111' }
  let(:harvest_credentials) { { 'accessToken' => 'access_token', 'accountId' => 'account_id', 'userId' => user_id } }
  ...
  context 'when external link compatible ARI provided' do
    ...
    context 'when multiple compatible ARIs' do
      it 'aborts with correct message' do
        ...
        expect { cli.perform }.to raise_error(Abt::Cli::Abort, 'Got reference data from multiple scheme providers, only one is supported at a time')
      end
    end
  end
  ...
  context 'when missing project_id' do
    it 'aborts with correct error message' do
      ...
      expect { cli.perform }.to raise_error(Abt::Cli::Abort, 'No current/specified project. Did you initialize Harvest and pick a task?')
    end
  end
  ...
  context 'when missing project_task' do
    it 'aborts with correct error message' do
      ...
      expect { cli.perform }.to raise_error(Abt::Cli::Abort, 'No current/specified task. Did you pick a Harvest task?')
    end
  end
end
