...
RSpec.describe Abt::Providers::Git::Commands::Branch do
  ...
  it 'switches to the branch with the branch name provided a by compatible ARI' do
    ...
    allow(Open3).to receive(:popen3).with('git switch branch-name') do |&block|
      block.call(nil, nil, nil, thread_double(true))
    end
  ...
  context 'when the branch does not exist' do
    it 'allows lets the user create the branch' do
      ...
      allow(Open3).to receive(:popen3).with('git switch branch-name') do |&block|
        block.call(nil, nil, nil, thread_double(false))
      end

      # Mock successful create and switch call
      allow(Open3).to receive(:popen3).with('git switch -c branch-name') do |&block|
        block.call(nil, nil, nil, thread_double(true))
      end
